    ...
    this.messagesGroupedByDate$ = value$.pipe(
      map((messages) => {
        this.logService.debug(
          `CHMCC: Raw messages input to map (contact: ${this.contact?.jid.toString()}):`,
          {
            count: messages.length,
        ...
        if (messages.length === 0) {
          this.logService.debug(
            `CHMCC: No messages to display (contact: ${this.contact?.jid.toString()})`
          );
          return [];
        }
        ...
        this.logService.debug(
          `CHMCC: Grouped messages output from map (contact: ${this.contact?.jid.toString()}):`,
          {
